"""
QFT Circuit Benchmarking Module

Benchmarks for Quantum Fourier Transform circuits.
"""
...
def _run_single_iteration(args):
    ...
    """
    iteration, num_qubits, num_partitions, algo_config, qft_config = args
    ...
    circuit = QFT(
        num_qubits,
        do_swaps=qft_config.get('do_swaps', False)
    )
...
def run_qft_benchmark(config, output_dir):
    """
    Run benchmarks for QFT circuits.
    ...
    for num_qubits, num_partitions in tqdm(configs_to_run, desc=f"{circuit_type} Benchmarks"):
        ...
        iteration_args = [
            (iteration, num_qubits, num_partitions, algo_config, qft_config)
        ...
        with open(means_filename, "w") as f:
            ...
            print(f"  Mean Cost: {mean_cost:.3f}, Mean Time: {mean_time:.3f}s")
    
    print(f"\nResults saved to {detailed_filename} and {means_filename}")

# Run main and get config from YAML
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run QFT Circuit Benchmarks")
    parser.add_argument(
        "--config",
        type=str,
    ...
    # Run benchmark
    run_qft_benchmark(config, args.output_dir)