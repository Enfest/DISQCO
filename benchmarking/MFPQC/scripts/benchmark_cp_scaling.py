"""
CP Circuit Benchmarking - Scaling Module

Benchmarks for CP fraction circuits with scaling partitions (2-12).
"""
...
def _run_single_iteration(args):
    ...
    """
    iteration, num_qubits, num_partitions, fraction, algo_config = args
...
def run_cp_scaling_benchmark(config, output_dir):
    """
    Run CP circuit benchmarks with scaling partitions (2-12).
    
    Args:
        config: Configuration dictionary
        output_dir: Directory to save results
    """
    cp_config = config['cp_scaling']
    algo_config = config['algorithm']
    num_iterations = config['num_iterations']
    num_processes = config.get('num_processes', 1)
    
    detailed_filename = os.path.join(output_dir, "benchmark_results_MLFM-R_CP_scaling.json")
    means_filename = os.path.join(output_dir, "benchmark_means_MLFM-R_CP_scaling.json")
    ...
    sizes = cp_config['sizes']
    fractions = cp_config['fractions']
    ...
    for num_qubits, num_partitions, fraction in tqdm(configs_to_run, desc="CP Scaling Benchmarks"):
        # Prepare arguments for parallel execution
        iteration_args = [
            (iteration, num_qubits, num_partitions, fraction, algo_config)
        ...
        if num_processes > 1:
            with Pool(processes=num_processes) as pool:
                iteration_data = list(tqdm(
                    ...
                    desc=f"  {num_qubits}q/{num_partitions}p/f={fraction}",
                    leave=False
                ))
        else:
            iteration_data = [
        ...
        tqdm.write(f"\n{num_qubits}q/{num_partitions}p/f={fraction}: Mean Cost={mean_cost:.3f}, Mean Time={mean_time:.3f}s")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run CP Scaling Circuit Benchmarks")
    parser.add_argument(
        "--config",
        type=str,
    ...
    # Run benchmark
    run_cp_scaling_benchmark(config, args.output_dir)