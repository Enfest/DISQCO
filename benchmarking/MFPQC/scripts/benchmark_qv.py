"""
Quantum Volume Circuit Benchmarking Module

Benchmarks for Quantum Volume circuits.
"""
...
def _run_single_iteration(args):
    ...
    """
    iteration, num_qubits, num_partitions, algo_config = args
...
def run_qv_benchmark(config, output_dir):
    """
    Run benchmarks for Quantum Volume circuits.
    ...
    for num_qubits, num_partitions in tqdm(configs_to_run, desc=f"{circuit_type} Benchmarks"):
        ...
        iteration_args = [
            (iteration, num_qubits, num_partitions, algo_config)
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run QV Scaling Circuit Benchmarks")
    parser.add_argument(
        "--config",
        type=str,
    ...
    # Run benchmark
    run_qv_benchmark(config, args.output_dir)