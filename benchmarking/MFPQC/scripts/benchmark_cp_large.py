"""
CP Circuit Benchmarking - Large Scale Module

Benchmarks for large CP fraction circuits (112-240 qubits) with 2-4 partitions.
"""
...
def run_cp_large_benchmark(config, output_dir):
    """
    Run CP circuit benchmarks for large scale (112-240 qubits, 2-4 partitions).
    
    Args:
        config: Configuration dictionary
        output_dir: Directory to save results
    """
    cp_config = config['cp_large']
    algo_config = config['algorithm']
    num_iterations = config['num_iterations']
    num_processes = config.get('num_processes', 1)
    
    detailed_filename = os.path.join(output_dir, "benchmark_results_MLFM-R_CP_large.json")
    means_filename = os.path.join(output_dir, "benchmark_means_MLFM-R_CP_large.json")
    ...
    num_partitions_list = cp_config['num_partitions']
    fraction = cp_config['fraction']
    ...
    for num_qubits, num_partitions in tqdm(configs_to_run, desc="CP Large Benchmarks"):
        ...
        tqdm.write(f"\n{num_qubits}q/{num_partitions}p: Mean Cost={mean_cost:.3f}, Mean Time={mean_time:.3f}s")
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Run CP Large Circuit Benchmarks")
    parser.add_argument(
        "--config",
        type=str,
    ...
    # Run benchmark
    run_cp_large_benchmark(config, args.output_dir)