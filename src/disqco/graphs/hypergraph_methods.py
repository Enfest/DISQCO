    ...
    if dummy_nodes == {}:
        ...
        for root_node in root_set:
            ...
            if root_node in dummy_nodes:
                partition_root = node_map[root_node[2]]
                root_counts[partition_root] += 1
                continue
        ...
        for rec_node in receiver_set:
            ...
            if rec_node in dummy_nodes:
                partition_rec = node_map[rec_node[2]]
                rec_counts[partition_rec] += 1
                continue
...
def map_hedge_to_configs(hypergraph,hedge,assignment,num_partitions, node_map=None, dummy_nodes = {}):
    root_counts,rec_counts = hedge_k_counts(hypergraph,hedge,assignment,num_partitions,set_attrs=False, node_map=node_map, dummy_nodes=dummy_nodes)
...
def hedge_to_cost_hetero(hypergraph : QuantumCircuitHyperGraph, 
    ...
    Computes the cost of a hyperedge based on its configuration and the current assignment."
    """
    root_config, rec_config = map_hedge_to_configs(hypergraph, hedge, assignment, num_partitions, dummy_nodes=dummy_nodes)
...
def map_counts_and_configs(hypergraph : QuantumCircuitHyperGraph, 
    ...
    if hetero:
        ...
        return map_counts_and_configs_hetero(hypergraph, 
                                             assignment, 
                                             num_partitions, 
                                             network, 
                                             costs=costs, 
...
def map_counts_and_configs_hetero(hypergraph : QuantumCircuitHyperGraph,
                                  ...
                                  costs: dict = {},
                                  node_map = None,
                                  dummy_nodes = set()) -> None:
    """
    Maps the counts and configurations of all hyperedges to hyperedge attributes based on the current assignment.
    For heterogeneous networks, it uses the network to compute the costs.
    """
    for edge in hypergraph.hyperedges:
        root_counts, rec_counts = hedge_k_counts(hypergraph,
                                                 edge,
                                                 assignment,
                                                 num_partitions,
                                                 set_attrs=True,
...
def calculate_full_cost_hetero(hypergraph : QuantumCircuitHyperGraph, 
    ...
    for edge in hypergraph.hyperedges:
        root_counts, rec_counts = hedge_k_counts(hypergraph, 
                                                 ...
                                                 dummy_nodes=dummy_nodes)
        root_config, rec_config = counts_to_configs(root_counts, rec_counts)
        edge_info = hypergraph.hyperedges[edge]

        # print(f"edge: {edge}, info: {edge_info}")

        # print(f"root_config: {root_config}, rec_config: {rec_config}")
        ...
        if (root_config, rec_config) in costs:
            ...
            costs[(root_config, rec_config)] = edge_cost
        cost += edge_cost
        # print(f"Edge {edge} cost: {edge_cost}")
